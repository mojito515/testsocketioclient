...
webSocketConnect(function(a) {
    a.onmessage = function(i) {
        ...
        if (h.op == "minitx") {
            ...
            var c;
            if (b.tag) {
                c = '<a href="' + root + "tx/" + b.hash + '" class="tag-address" style="width:85px">' + b.hash + '</a> <span class="tag">(' + b.tag;
                if (b.tag_link) {
                    c += ' <a class="external" rel="nofollow" href="' + root + "r?url=" + encodeURI(b.tag_link) + '" target="new"></a>'
                }
                c += ")</span>"
            } else {
                c = '<a href="' + root + "tx/" + b.hash + '">' + b.hash.substring(0, 25) + "...</a>"
            }
            var f = $("<tr><td><div>" + c + '</div></td><td class="hidden-phone" data-time="' + b.time + '"><div>< 1 minute</div></td><td><div><button class="btn btn-success cb">' + formatMoney(b.value, true) + "</button></div></td></tr>");
            ...
            if (h.op == "block") {
                ...
                var g = "Unknown";
                if (j.foundBy != null) {
                    g = '<a href="' + j.foundBy.link + '">' + j.foundBy.description + "</a>"
                }
                if (j.txIndex) {
